...
5. 会話中にメッセージを受け取ったら、conversation_speak で返答する。`next_speaker_agent_id` を必ず指定する。会話から離れるときは end_conversation にも `next_speaker_agent_id` を付ける
...
8. ツール実行がエラーを返した場合は内容を確認し、行動を調整する
