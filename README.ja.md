...
- 操作と通知の窓口は複数あります
  ...
  - ブラウザ UI 向けの publish 済み snapshot object と `/api/history`、およびその背後で使う任意の backend 側 snapshot / WebSocket publisher 入力
...
ブラウザで動くダッシュボードや観戦 UI は、次を利用します。

- current-state polling 用の publish 済み snapshot object URL（`VITE_SNAPSHOT_URL`）
- タイムライン / 詳細オーバーレイ用の Worker `/api/history`

`GET /api/snapshot` と `GET /ws` は、観戦 SPA が直接契約する browser-facing endpoint ではなく、publisher / 任意 relay 層が使う backend/admin 側の source endpoint です。current-state 観戦 UI の primary path は、fixed-cadence の snapshot を R2 カスタムドメイン上の object として publish し、それをブラウザが直接 polling する構成です。`GET /ws` は任意の accelerator / debugging feed であり、観戦 UI の freshness readiness を成立させるための必須経路ではありません。

これらの backend 側 source は、publisher worker、管理ツール、観測クライアント、デバッグ用途では引き続き利用できます。

`karakuri-world-ui/` の観戦 SPA は `npm run dev` / `npm run build` のどちらでも、次の Vite env が必須です。

- `VITE_SNAPSHOT_URL`: ブラウザが直接取得する snapshot object の絶対 URL
- `VITE_AUTH_MODE`: `public` または `access`
- `VITE_API_BASE_URL`: Worker history endpoint の絶対 URL。末尾まで `/api/history` である必要があります

これらの URL はブラウザ bundle に露出するため、認証情報・query parameter・fragment を埋め込まないでください。追跡可能な契約詳細は [`docs/plan/12-ui-app-shell-bootstrap.md`](./docs/plan/12-ui-app-shell-bootstrap.md) と [`docs/design/detailed/15-ui-application-shell.md`](./docs/design/detailed/15-ui-application-shell.md) を参照してください。
