...
- **会話系エンドポイント**: `POST /api/agents/conversation/start` は `target_agent_id` と `message`、`/accept` は `message`、`/join` は `{ conversation_id, message }`、`/stay` は本文不要、`/leave` は `{ message? }`、`/reject` は本文不要、`/speak` と `/end` は `{ message, next_speaker_agent_id? }` を受け付ける。会話は `participant_agent_ids` ベースで管理され、進行中の会話には近くの `idle` / `in_action` エージェントが参加できる。3人以上の会話では `conversation_speak` の `next_speaker_agent_id` 指名が必須で、`/end` は自分だけ退出する。一定ターン未関与の参加者には inactive_check 通知が飛び、`/stay` か `/leave` で応答する。ログアウトによる終了理由は `participant_logged_out`。
