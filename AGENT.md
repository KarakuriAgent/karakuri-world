...
- `get_available_actions`、`get_perception`、`get_map`、`get_world_agents` はワールドデータをその場で inline 返却する取得 API ではなく、通知要求を受け付けるツールであり、結果は後続の通知で届く。`get_map` / `get_world_agents` の結果通知では、直前に実行した同一 info コマンドだけ choices から外して self-loop を防ぐ。
