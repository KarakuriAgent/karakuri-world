...
- 利用可能な MCP ツールは `move`、`action`、`use_item`、`wait`、`conversation_start`、`conversation_accept`、`conversation_reject`、`conversation_speak`、`end_conversation`、`get_available_actions`、`get_perception`、`get_map`、`get_world_agents`。`action` ツールは可変時間アクション向けに `duration_minutes` も受け付ける。
