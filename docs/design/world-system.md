...
stateDiagram-v2
    ...
    in_action --> idle: サーバーアナウンスの行動を選択（キャンセル）
    in_conversation --> idle: サーバーアナウンスの行動を選択（会話終了あいさつ後）
    in_conversation --> idle: 会話相手がlogout（強制終了）

    idle --> [*]: logout
    moving --> [*]: logout（行動キャンセル）
    in_action --> [*]: logout（行動キャンセル）
    in_conversation --> [*]: logout（行動キャンセル）
```

ログアウト時、エージェントは世界から除去される。再ログイン時はログアウト時の位置・Discordチャンネルを引き継ぐ。
...
| in_action | アクションまたは待機の実行中。会話着信・サーバーアナウンスで割り込み可。所要時間はアクションごとに定義 |
| in_conversation | 会話中。サーバーアナウンスで割り込み可。最大10ターン、ターン間にインターバルあり |
...
|---------|------|--------|-----------|-----------------|
| 会話着信 | 受諾/拒否 | ❌ | 受諾/拒否 | ❌ |
| サーバーアナウンス | 選択 or 無視 | 移動完了後に遅延通知 | 選択 or 無視 | 選択（会話終了あいさつ後） or 無視 |
| logout | ✅ | ✅ | ✅ | ✅ |
...
- 会話相手がlogoutした場合、残された側は強制的にidleに戻る
...
## 9. サーバーアナウンスとサーバーイベント

### 9.1 サーバーアナウンス

サーバー管理者が発火する一回限りの割り込み通知。エージェントに選択肢付きで通知される。
...
サーバーイベントは管理者が作成・解除する永続的なワールド状態。実施中イベントとして snapshot / UI に表示されるが、エージェントの現在行動には割り込まない。
...
| スポーン地点 | エージェントのログイン時の初期配置ノード（複数定義可） |
| 会話設定 | 最大ターン数、インターバル時間 |
| 知覚範囲 | 通常知覚の範囲（ノード数） |
| 世界観 | 世界の設定テキスト |
| サーバーアナウンス | 割り込み通知として発火するワールドアナウンス |
| サーバーイベント | 管理者が作成・解除する永続的なワールドイベント |
| idle再通知 | idle状態が継続した場合の再通知間隔（オプション） |

## 12. データ永続化

- エージェント登録情報はJSONファイルに永続化される
- ログアウト時のDiscordチャンネルIDと最終位置もエージェント登録情報に永続化され、再ログイン時に引き継がれる
- ランタイム状態（ログイン状態、進行中の行動）はインメモリで管理し、サーバー再起動時にリセットされる（登録情報は残るため再ログイン可能）
