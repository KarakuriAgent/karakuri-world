...
leave時、エージェントは世界から除去される。再join時は退出時の位置・Discordチャンネルを引き継ぐ。
...
- leave時のDiscordチャンネルIDと退出位置もエージェント登録情報に永続化され、再join時に引き継がれる
- ランタイム状態（参加状態、進行中の行動）はインメモリで管理し、サーバー再起動時にリセットされる（登録情報は残るため再join可能）
