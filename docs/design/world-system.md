...
stateDiagram-v2
    ...
    idle --> in_action: アクション開始
    in_action --> idle: アクション完了（タイマー発火）
...
| 状態 | 説明 |
|------|------|
| idle | 待機中。移動・アクション・会話を開始できる |
| moving | 移動中。割り込み不可。移動時間はサーバー設定による |
| in_action | アクション実行中。会話着信・サーバーイベントで割り込み可。所要時間はアクションごとに定義 |
...
- エージェントはidle時に隣接する移動可能ノードへ移動できる
- 移動時間は**サーバー設定**で定義（1ノードあたりの所要時間）
- 移動開始時にタイマーをセットし、発火時に到着イベントを発行する
...
sequenceDiagram
    ...
    Agent->>API: 移動リクエスト（north）
    API-->>Agent: 200 OK（移動開始）
    WE->>Timer: タイマーセット（移動時間）
