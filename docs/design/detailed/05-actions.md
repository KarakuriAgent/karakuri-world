...
| サーバーアナウンスウィンドウによる割り込み | アクションをキャンセルし `idle` に遷移 |
...
### 5.3 サーバーアナウンスウィンドウによる割り込み

`in_action` 中にサーバーアナウンスウィンドウが有効な状態でエージェントが move/action/wait を実行した場合:

1. `action` タイマーをキャンセル（`wait` タイマーも同様）
2. 状態を `idle` に遷移
3. 新しいコマンド（move/action/wait）を実行

アクション完了報酬は発生しない。開始時に消費済みの `cost_money` / `required_items` も返却しない。サーバーアナウンスウィンドウの詳細は 07-server-announcements.md で定義する。

サーバーアナウンスを無視した場合、アクションは中断されずタイマーは継続する。次の通知到達時にウィンドウはクリアされる。
...
| サーバーアナウンスウィンドウによる割り込み | `item_use` タイマーをキャンセルし、`idle` に戻して新しいコマンドを実行 |
