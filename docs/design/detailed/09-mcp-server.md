...
認証成功後、ログイン必須のツール（`move`、`action`、`conversation_*`、`end_conversation`、`server_event_select`、`get_*`）を未ログイン状態で呼び出した場合は、ツール実行エラー（セクション3.2）として `not_logged_in` エラーを返す。これはREST APIの `403 Forbidden`（08-rest-api.md セクション1.3）に相当する。
