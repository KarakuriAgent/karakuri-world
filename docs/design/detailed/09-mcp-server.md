...
ライフサイクル操作（`POST /api/agents/login`、`POST /api/agents/logout`）はユーザーまたは運用スクリプトが実行するため、MCPツールとして公開しない。`move` / `action` / `wait` は通常は `idle` 状態でのみ成功するが、アクティブなサーバーイベント通知の割り込みウィンドウ中だけは `in_action` / `in_conversation` からも実行できる。
...
```json
{
  ...
  "description": "会話中に発言する。in_conversation状態で自分のターンのときのみ実行可能。3人以上の会話では next_speaker_agent_id が必須。",
  "inputSchema": {
    "type": "object",
    "properties": {
      "message": {
      ...
      "next_speaker_agent_id": {
        "type": "string",
        "description": "3人以上の会話で次に発言する参加者のID"
      }
    },
    "required": ["message"]
  }
...
### 2.8 conversation_join

```json
{
  "name": "conversation_join",
  "description": "近くで進行中の会話に参加する。",
  "inputSchema": {
    "type": "object",
    "properties": {
      "conversation_id": {
...
### 2.11 end_conversation

```json
{
  "name": "end_conversation",
  "description": "会話を自発的に終了または退出する。2人会話では終了要求、3人以上では自分だけ退出する。",
  "inputSchema": {
    "type": "object",
    "properties": {
      "message": {
...
認証成功後、ログイン必須のツール（`move`、`action`、`conversation_*`、`end_conversation`、`get_*`）を未ログイン状態で呼び出した場合は、ツール実行エラー（セクション3.2）として `not_logged_in` エラーを返す。これはREST APIの `403 Forbidden`（08-rest-api.md セクション1.3）に相当する。
