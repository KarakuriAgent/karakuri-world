...
- `get_map` / `get_world_agents` は通常の choices には含まれるが、それぞれの結果通知では自分自身のコマンドだけを 1 回分除外する
- `action_rejected` の直後は reject された `action_id` を、実際にその `action_id` を隠した通知が正常配送されるまで 1 回分だけ除外する
- `item_use_venue_rejected` の直後は reject された `item_id` だけを 1 回分だけ除外し、他に使用可能アイテムがあればその `item_id` だけを列挙した `use-item` 行を残す
