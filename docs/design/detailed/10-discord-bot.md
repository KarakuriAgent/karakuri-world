...
- `conversation_accepted`, `conversation_message`, `conversation_join`, `conversation_leave`, `conversation_inactive_check`, `conversation_turn_started`, `conversation_closing`, `conversation_rejected`, `conversation_ended`, `conversation_pending_join_cancelled`
- `server_event_fired`
...
- conversation_join: 会話に参加する (conversation_id: 会話ID)
...
| 7.6 | ターン再開通知 | `conversation_turn_started` | — | あり |
...
`reason: "participant_logged_out"` の場合は「参加予定だった会話が、参加者のログアウトにより開始前に終了しました。」という専用文面で通知する。それ以外の `reason` では #9 と同じ「会話が終了しました。（{reason_text}）」文面を流用する（joiner 本人は発言機会を持たないまま終了したことになる）。
