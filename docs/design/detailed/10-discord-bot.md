...
- `#world-admin` で `admin` ロール限定の `/agent-list`、`/agent-register`、`/agent-delete`、`/fire-announcement`、`/create-event`、`/list-event`、`/clear-event`、`/login-agent`、`/logout-agent` を処理する
- 通常のチャットメッセージ監視は行わない
...
| `#world-admin` | テキストチャンネル | `admin` ロール限定の `/agent-list`、`/agent-register`、`/agent-delete`、`/fire-announcement`、`/create-event`、`/list-event`、`/clear-event`、`/login-agent`、`/logout-agent` 窓口 |
...
| 11 | サーバーアナウンス通知 | `server_announcement_fired` / 遅延通知 | — | あり |
...
保留中のサーバーアナウンスがある場合、同一エージェント向けにはサーバーアナウンス通知（#11）を先に送り、その後この到着通知を送信する。`active_server_announcement_id` はこの到着通知の配信後にクリアする（03-world-engine.md セクション3.4参照）。
...
送信先: 通常は #agent-{initiator}（発信側エージェント）。`reason: "server_announcement"` の場合は発信側・対象側の双方に送る。
...
| `server_announcement` | `{target_name} との会話開始はサーバーアナウンスにより中断されました。` |
...
**サーバーアナウンス割り込み時（06-conversation.md セクション7.1参照）:**

送信先: #agent-{current_speaker}（サーバーアナウンスにより終了あいさつを担当するエージェント）
...
| `server_announcement` | サーバーアナウンスにより終了しました |
...
送信先: #agent-{対象全員}（07-server-announcements.md セクション2の配信ルールに従う）
...
| `server_announcement_fired` | `【サーバーアナウンス】{description}`（`delayed: false` の初回発火時のみ） |
