...
- `conversation_accepted`, `conversation_message`, `conversation_join`, `conversation_leave`, `conversation_inactive_check`, `conversation_turn_started`, `conversation_closing`, `conversation_rejected`, `conversation_ended`
- `server_event_fired`
...
- conversation_join: 会話に参加する (conversation_id: 会話ID, message: 最初のメッセージ)
...
選択肢:
- conversation_speak: 返答する (message: 発言内容[, next_speaker_agent_id])
- end_conversation: 会話を終了 / 退出する (message: お別れのメッセージ[, next_speaker_agent_id])

{skill_name} スキルで次の行動を選択してください。
```

3人以上の会話では `next_speaker_agent_id` の指定が必要。
