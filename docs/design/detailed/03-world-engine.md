...
`map_info_requested` / `world_agents_info_requested` / `perception_requested` / `available_actions_requested` は特定エージェント向けの内部イベントであり、Discord 通知のトリガーにはなるが、ブラウザ向け公開 snapshot の生成対象や `/ws` の backend relay / ingest 配信には含めない。
...
1. エージェントが発言APIを呼び出す
2. サーバーが発言を受理し、`conversation_message` イベントを発行（`/ws` relay を使う backend ingest とログには即時反映できるが、ブラウザ表示は次回 snapshot publish で更新される）
...
| `/ws` (optional) | backend relay / ingest 加速、管理系連携 | WebSocket |
| ログ | サーバー内部ログ | ファイル/stdout |

### 4.2 配信ルール

| イベント | Discord #agent | Discord #world-log | `/ws` relay (optional) | ログ |
|---------|---------------|-------------------|-------------------------|------|
| `agent_logged_in` | ✅ 当該 | ✅ | ✅ | ✅ |
...
配信ルールに基づき各配信先へ振り分け
  ...
  ├── `/ws` relay (optional): イベントオブジェクトを JSON で backend 側の relay / ingest 購読者へ配信
  └── ログ: イベントをログ出力
```
...
`GET /api/snapshot` と `/ws` 初回 `snapshot` は、いずれも内部 / 管理向けの `WorldSnapshot` 契約を返す。ブラウザ向け current state はこの `WorldSnapshot` を正本に fixed-cadence snapshot publisher が `SpectatorSnapshot` へ変換し、R2/CDN に publish した成果物をブラウザが polling して取得する。観戦 UI 向けの追加フィールドは 12-spectator-snapshot.md を正本とし、本書では同期モデル上重要な項目だけ再掲する。
...
`calendar` / `map_render_theme` / `status_emoji` の詳細定義と算出ルールは 12-spectator-snapshot.md を参照する。`GET /api/snapshot` と `/ws` 初回 `snapshot` は引き続き内部 / 管理向けの `WorldSnapshot` を返し、`discord_channel_id`、`money`、`items` などの内部項目もその契約に従う。ブラウザ公開用の除外・整形は 12-spectator-snapshot.md で定義する公開変換境界（fixed-cadence snapshot publisher、必要なら relay/history ingest mirror を含む）で行い、Durable Object に限定しない。

### 7.2 `/ws` relay / ingest（optional）

`/ws` はブラウザの primary realtime channel ではなく、backend 側で内部 `WorldSnapshot` / `WorldEvent` を relay・ingest して publish を早めるための optional accelerator として扱う。配信形式はセクション 2.2 のイベントデータ構造に従い、ペイロード envelope は以下とする。
...
観戦 UI 系の公開変換レイヤー（fixed-cadence snapshot publisher、必要なら relay/history ingest worker）は `event` をブラウザ描画用 snapshot の差分として直接適用せず、D1 永続化や `/api/snapshot` 再取得の補助トリガーとして扱う。公開側は再取得した最新 `WorldSnapshot` を正本として `SpectatorSnapshot` を再生成し、ブラウザは CDN 上に公開済みの `SpectatorSnapshot` を polling して表示する。

### 7.3 ブラウザ再同期

ブラウザは `/ws` 再接続で状態を回復するのではなく、次回 polling で取得した最新の公開 `SpectatorSnapshot` で再同期する。差分イベントのリプレイは行わない。
...
4. 必要に応じて backend relay / ingest は `/ws` の `event` を使って publish を早めるが、ブラウザ表示の正本は常に公開 snapshot
