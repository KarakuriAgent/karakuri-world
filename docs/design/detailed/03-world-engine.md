...
サーバーアナウンスは専用タイマーを持たず、`active_server_announcement_id` フラグでウィンドウを管理する。
...
| `action` | エージェントlogout、会話受諾（`in_action` からの遷移）、サーバーアナウンスウィンドウによる割り込み |
| `wait` | エージェントlogout、会話受諾（`in_action` からの遷移）、サーバーアナウンスウィンドウによる割り込み |
| `item_use` | エージェントlogout、会話受諾（`in_action` からの遷移）、サーバーアナウンスウィンドウによる割り込み |
| `conversation_accept` | 相手が受諾、相手が拒否、発信側logout、対象側logout |
| `conversation_turn` | 対象エージェントが発言、会話終了（max_turns到達、相手logout、サーバーアナウンスウィンドウによる割り込み） |
| `conversation_interval` | 会話終了（max_turns到達、相手logout、サーバーアナウンスウィンドウによる割り込み） |
| `conversation_inactive_check` | 対象エージェント全員が `stay` / `leave` で応答、会話終了、相手logout、サーバーアナウンスウィンドウによる割り込み |
...
| `conversation_turn` | `active` 状態: 会話を終了、両者を `idle` に遷移、`conversation_ended` イベント発行（`reason: "turn_timeout"`）。`closing` 状態: 終了あいさつ未送信として会話を終了、`conversation_ended` イベント発行（`reason` は終了理由に応じて `"max_turns"`、`"server_announcement"`、または `"ended_by_agent"`）。詳細は 06-conversation.md セクション5.2、7.1、7.2 |
...
| `conversation_leave` | 会話離脱 | 離脱API、inactive_check タイムアウト、logout、サーバーアナウンス割り込み |
...
| `conversation_closing` | 会話が終了あいさつフェーズに移行 | max_turns到達、サーバーアナウンスウィンドウによる割り込み、自発終了 |
| `conversation_ended` | 会話終了 | max_turns到達（終了あいさつ後）、ターンタイムアウト、サーバーアナウンスウィンドウによるclosing完了後、エージェントによる自発終了（終了あいさつ後）、相手logout |
| `conversation_pending_join_cancelled` | 保留中の参加取り消し通知 | 会話全体が終了する際、`pending_participant_agent_ids` に残っていた joiner 毎に1件発火 |
| `server_announcement_fired` | サーバーアナウンス発生 | 管理API `POST /api/admin/server-announcements/fire` |
| `server_event_created` | 永続サーバーイベント開始 | 管理API `POST /api/admin/server-events` / Discord `/create-event` |
| `server_event_cleared` | 永続サーバーイベント解除 | 管理API `DELETE /api/admin/server-events/:event_id` / Discord `/clear-event` |
| `server_events_info_requested` | 実施中サーバーイベント情報取得依頼 | `get_event` API/MCP |
...
interface MovementCompletedEvent extends EventBase {
  ...
  agent_name: string;
  node_id: NodeId;
  delivered_server_announcement_ids: string[]; // 移動完了時に遅延通知したサーバーアナウンスID一覧
}
...
interface ConversationRejectedEvent extends EventBase {
  ...
  initiator_agent_id: string;
  target_agent_id: string;
  reason: "rejected" | "timeout" | "target_logged_out" | "server_announcement";
}
...
interface ConversationClosingEvent extends EventBase {
  ...
  reason: "max_turns" | "server_announcement" | "ended_by_agent";
}
...
interface ConversationEndedEvent extends EventBase {
  ...
  initiator_agent_id: string;
  participant_agent_ids: string[];
  reason: "max_turns" | "turn_timeout" | "server_announcement" | "ended_by_agent" | "participant_logged_out";
  final_message?: string;           // 終了あいさつの発言内容（max_turns・server_announcement・ended_by_agent時）
  final_speaker_agent_id?: string;  // 終了あいさつの発言者
  // participant_agent_ids に含まれる参加者へ配信する。
  // max_turns・server_announcement・ended_by_agent の場合、終了あいさつフェーズを経てから発行される（詳細は 06-conversation.md）
}
...
interface ConversationPendingJoinCancelledEvent extends EventBase {
  ...
  reason: "max_turns" | "turn_timeout" | "server_announcement" | "ended_by_agent" | "participant_logged_out" | "agent_unavailable";
  // 会話全体が終了した際、pending_participant_agent_ids に残っていた joiner 毎に発行される。
  // 対象の joiner 本人へ follow-up 通知を送り、#world-log には投稿しない。
}

interface ServerAnnouncementFiredEvent extends EventBase {
...
interface ServerEventCreatedEvent extends EventBase {
  type: "server_event_created";
  server_event: ServerEvent; // active 確定（cleared_at === null）
}

interface ServerEventClearedEvent extends EventBase {
  type: "server_event_cleared";
  server_event: ServerEvent; // cleared 確定（cleared_at !== null）
}

interface ServerEventsInfoRequestedEvent extends EventBase {
  type: "server_events_info_requested";
  agent_id: string;
}

サーバーアナウンス専用の選択 / 期限切れイベントは廃止され、`server_announcement_fired` と `active_server_announcement_id` でウィンドウを表現する。永続サーバーイベントは `server_event_created` / `server_event_cleared` で世界全体の状態遷移のみを通知し、エージェントの状態（`in_action` / `in_conversation` / `in_transfer`）には介入しない。
...
| サーバーアナウンス通知 | `server_announcement_fired` | #agent-{対象全員} | 説明文、強制表示された行動候補、行動促進 |
| サーバーアナウンス遅延通知 | `movement_completed`（保留あり） | #agent-{name} | 説明文、強制表示された行動候補、行動促進 |
| サーバーアナウンスログ | `server_announcement_fired` | #world-log | 説明文 |
...
- ※5 `max_turns` 到達時は最終ターンの聞き手へ、サーバーアナウンスウィンドウによる割り込み時は終了あいさつ担当へ送信（06-conversation.md セクション7.1、7.2参照）
...
### 3.4 moving中のサーバーアナウンス遅延通知

`moving` 状態のエージェントにはサーバーアナウンスを即座に通知せず、移動完了後に遅延通知する。

1. `server_announcement_fired` 発生時、`moving` のエージェントのアナウンスIDを保留リストに記録
2. `movement_completed` 時に保留リストを確認
3. 保留中のサーバーアナウンスがあれば、移動完了通知と合わせて遅延通知を送信（複数の保留がある場合はすべて通知する）
4. 各遅延通知では `active_server_announcement_id` をセットし、pending がなくなったらインスタンスを削除する
5. 遅延 `server_announcement_fired` の後も割り込みウィンドウは維持し、次のエージェント向け通知（通常は `movement_completed`）で `active_server_announcement_id` をクリアする
...
| `server_announcement_fired` | ✅ 対象全員 ※3 | ✅ | ✅ | ✅ |
...
| サーバーアナウンス保留リスト | 保留中のサーバーアナウンスを破棄 |
...
`WorldEngine.getSnapshot()` は内部 / 管理向けの `WorldSnapshot` を返す in-process API で、event-driven snapshot publisher がこれを呼び出して `SpectatorSnapshot` に変換し、`POST /api/publish-snapshot` で relay Worker に body push、Worker が R2/CDN に publish した成果物をブラウザが polling して取得する。pull 用 `GET /api/snapshot` HTTP endpoint はどのレイヤーにも提供しない（push が唯一の配信経路）。観戦 UI 向けの追加フィールドは 12-spectator-snapshot.md を正本とし、本書では同期モデル上重要な項目だけ再掲する。
...
interface WorldSnapshot {
  ...
  recent_server_announcements: RecentServerAnnouncementSnapshot[];  // 直近のサーバーアナウンス（最大10件の FIFO リングバッファ）
...
interface RecentServerAnnouncementSnapshot {
  server_announcement_id: string;
  description: string;
  occurred_at: number;
...
`calendar` / `map_render_theme` / `status_emoji` の詳細定義と算出ルールは 12-spectator-snapshot.md を参照する。`WorldEngine.getSnapshot()` が返す内部 `WorldSnapshot` には `discord_channel_id`、`money`、`items` などの内部項目が含まれる。ブラウザ公開用の除外・整形は 12-spectator-snapshot.md で定義する公開変換境界（event-driven snapshot / history publisher）で行い、Durable Object に限定しない。
