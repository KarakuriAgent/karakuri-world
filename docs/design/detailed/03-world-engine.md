...
WebSocket 接続確立時の初回 `snapshot` と `GET /api/snapshot` は、同じ `WorldSnapshot` 契約を返す。観戦 UI 向けの追加フィールドは 12-spectator-snapshot.md を正本とし、本書では同期モデル上重要な項目だけ再掲する。
...
`calendar` / `map_render_theme` / `status_emoji` の詳細定義と算出ルールは 12-spectator-snapshot.md を参照する。`GET /api/snapshot` と `/ws` 初回 `snapshot` は引き続き内部 / 管理向けの `WorldSnapshot` を返し、`discord_channel_id`、`money`、`items` などの内部項目もその契約に従う。ブラウザ公開用の除外・整形は Durable Object が `WorldSnapshot` を `SpectatorSnapshot` へ変換する境界でのみ行う。

### 7.2 差分イベント配信

スナップショット送信後、サーバーは発生するイベントをリアルタイムで WebSocket クライアントへ配信する。配信形式はセクション 2.2 のイベントデータ構造に従い、ペイロード envelope は以下とする。
...
観戦 UI 系のクライアント（Durable Object）は `event` をブラウザ描画用 snapshot の差分として直接適用せず、D1 永続化と `/api/snapshot` 再取得のトリガーとして扱う。DO が再取得した最新 `WorldSnapshot` を正本として `SpectatorSnapshot` を再生成し、ブラウザはその公開済み `SpectatorSnapshot` を表示する。

### 7.3 再接続時の再同期

WebSocket 切断後の再接続時、サーバーは新しい `type: 'snapshot'` ペイロードを送信する。差分の追跡やイベントのリプレイは行わない。

再接続フロー:

1. WebSocket 接続の切断を検出
2. クライアントが再接続を試行
3. 接続確立時、サーバーが新しい `snapshot` を送信
4. クライアントはローカル状態を snapshot で置き換える
5. 以降の `event` を履歴蓄積・snapshot 再取得トリガーとして扱う
