...
| `conversation_interval` | `active` 状態: 次の発言者にDiscordで発言を配信し、必要なら `inactive_check` へ移行したうえで `conversation_turn` タイマーを生成する。turn が `ConversationConfig.max_turns` に到達した場合、または `closing_reason` が `"ended_by_agent"` の場合は終了あいさつフェーズに移行。`closing` 状態: 終了あいさつを配信し、参加者が残っていれば次の終了担当へターンを渡す。詳細は 06-conversation.md セクション5.2、6、7.1 |
...
- ※4 `reason: "participant_logged_out"` の場合、会話終了通知は送信しない。代わりに `agent_logged_out` をトリガーとする会話強制終了通知が残された側にのみ送信される（10-discord-bot.md セクション6.4 #10参照）
...
- ※4 `reason: "participant_logged_out"` の場合は会話終了通知を送信しない。残された側への通知は `agent_logged_out`（※1）の会話強制終了通知で行う
...
- `in_conversation` 中のlogoutでは、残された側のエージェントを `idle` に遷移させる。残り参加者が1人以下なら `conversation_ended`（`reason: "participant_logged_out"`）を発行し、2人以上残る場合は `conversation_leave` を発行して会話を継続する
