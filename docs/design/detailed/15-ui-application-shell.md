...
2. サーバーイベント: 原則 `snapshot.recent_server_events` の先頭 3 件。これが空で `snapshot.server_events` に active event がある場合は active event を fallback 表示し、完了済み履歴は未復元であることを示す
3. エージェント一覧: `snapshot.agents` を `state !== 'idle'` を先頭、同順位は `agent_name` 昇順で表示

`snapshot.server_events` は「現在 outstanding のイベント数」や active バッジ判定に使い、固定表示する「直近のサーバーイベント」は通常 `recent_server_events` を正本とする。ただし polling-only + cold start で cache が空の場合は completed history を約束せず、active event fallback または empty state を表示する。
