...
2. サーバーイベント: `snapshot.recent_server_events` の先頭 3 件を固定表示する（backend 側のリングバッファがそのまま publish される）。空の場合は empty state を表示する
3. エージェント一覧: `snapshot.agents` を `state !== 'idle'` を先頭、同順位は `agent_name` 昇順で表示

`recent_server_events` は `is_active` フラグ付きで完了済みイベントも含む。active バッジ判定は各エントリの `is_active` を使い、polling-only + cold start で cache が空の場合は empty state を表示する。
