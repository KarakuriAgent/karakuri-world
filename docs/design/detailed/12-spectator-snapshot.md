...
`generated_at` は本体がその `WorldSnapshot` を生成した時刻であり、event-driven publish 時の元データ時刻として扱う。`published_at` は snapshot publisher が公開 JSON を最後に正常反映した時刻で、`last_publish_error_at` などと並ぶ配信健全性診断の補助情報として使う。`recent_server_events` は backend 側 `WorldEngine.state.recentServerEvents` のリングバッファを写した「直近に観測・発火したイベント履歴」であり、完了済みイベント（`is_active=false`）も含む。cold start では publisher 起動前に完了した履歴の完全再現までは保証しない。
...
6. `recent_server_events` は `WorldSnapshot.recent_server_events`（backend 側のリングバッファ）をそのまま写し、`server_event_id` / `description` / `occurred_at` / `is_active` を保持する。`is_active` は backend 側の `maybeCleanupServerEvent` 実行状況を反映しており publisher 側での再計算は行わない
