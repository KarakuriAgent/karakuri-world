...
snapshot publisher が `WorldEngine.getSnapshot()` を正本の世界状態として再利用できるよう、`WorldSnapshot` に以下を追加する。固定表示する直近サーバーアナウンス履歴は 5.1 のとおり publisher-side cache から合成する。
...
`map_render_theme` は `apps/server/src/discord/map-renderer.ts` の定数を正本として生成する。UI 側で独自定数を持たず、Discord 表示とブラウザ表示の差分をなくす。特に `background_fill` と `node_id_font_size` を contract に含め、ブラウザ側へ固定値を残さない。
...
interface SpectatorRecentServerAnnouncement {
...
interface SpectatorActiveServerEvent {
...
interface SpectatorSnapshot {
  ...
  active_server_events?: SpectatorActiveServerEvent[]; // 旧 snapshot 互換のため optional
...
`generated_at` は本体がその `WorldSnapshot` を生成した時刻であり、event-driven publish 時の元データ時刻として扱う。`published_at` は snapshot publisher が公開 JSON を最後に正常反映した時刻で、`last_publish_error_at` などと並ぶ配信健全性診断の補助情報として使う。`recent_server_announcements` は backend 側 `WorldEngine.state.recentServerAnnouncements` のリングバッファを写した「直近に観測・発火したサーバーアナウンス履歴」であり、完了済みアナウンス（`is_active=false`）も含む。cold start では publisher 起動前に完了した履歴の完全再現までは保証しない。`active_server_events` は backend 側 `ServerEventStore.listActive()` を写した「現在実施中の永続サーバーイベント一覧」で、`cleared_at` は派生表現として publisher 側で除外する（active のみ載るため不要）。`active_server_events` を `optional()` にしているのは、リネーム前の旧 snapshot を receive 側 schema が引き続き parse できるようにするための後方互換措置である。

### 3.2 除外する情報

以下は `WorldSnapshot` / `WorldEvent` に存在しても `SpectatorSnapshot` と recent history publish 対象から除外する。R2 に publish される `history/*` object も同じ除外規則を共有する。
...
5. `conversations` は UI 描画に必要な要約項目だけへ圧縮する（`current_turn` や `initiator_agent_id` などの内部進行情報は除外する）
6. `recent_server_announcements` は `WorldSnapshot.recent_server_announcements`（backend 側のリングバッファ）をそのまま写し、`server_announcement_id` / `description` / `occurred_at` / `is_active` を保持する。`is_active` は backend 側の `maybeCleanupServerAnnouncement` 実行状況を反映しており publisher 側での再計算は行わない
7. `published_at` は publisher の書き込み時刻で上書きする
...
この方針により、現行本体の型定義（`apps/server/src/types/snapshot.ts` の `item_use` 分岐、`apps/server/src/types/timer.ts` の `ItemUseTimer`）と矛盾せずに実装できる。

### 5.3 `WorldEvent -> PersistedSpectatorEvent`

イベント永続化時も同じサニタイズ規則を適用する。`payload_json` / history document の `detail` フィールドは「イベント種別ごとの公開 allowlist だけを残した JSON」とし、以下を authoritative rule とする。
...
| `movement_completed` | `agent_id`, `agent_name`, `node_id`, `delivered_server_announcement_ids` | なし |
...
| `server_announcement_fired` | `server_announcement_id`, `description`, `delivered_agent_ids`, `pending_agent_ids`, `delayed` | なし |
...
`delayed` を `server_announcement_fired` の公開フィールドに含めるのは、observability 上の必須情報だからである。同じ `server_announcement_id` の遅延再配信を `server_announcement_instances.first_occurred_at` / `last_occurred_at` 差分で検出する設計（14-ui-history-api.md §4.3）と組み合わせ、運用者が遅延再配信の発生を後追い可能にする。なお `delayed` は UI 描画には使わず、`SpectatorRecentServerAnnouncement` には載せない。
...
詳細な recent history document 形式は 14-ui-history-api.md を参照。
