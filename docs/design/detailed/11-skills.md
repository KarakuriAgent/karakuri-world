...
環境変数 `KARAKURI_API_BASE_URL` と `KARAKURI_API_KEY` を読み込み、curlでREST APIを呼び出すラッパースクリプト。管理者用コマンドでは追加で `KARAKURI_ADMIN_KEY`（または `ADMIN_KEY`）を使う。サブコマンド形式で全APIコマンドをラップする。
...
| `status` | 所持金・所持品・現在地取得 |
| `nearby-agents` | 会話開始 / 譲渡に使える隣接エージェント候補取得 |
| `active-conversations` | 参加可能な進行中会話一覧取得 |
| `event` | 実施中サーバーイベント一覧の通知要求 |
| `event list [--include-cleared]` | 永続サーバーイベント一覧取得（管理者用） |
| `event create <description...>` | 永続サーバーイベント作成（管理者用） |
| `event clear <server_event_id>` | 永続サーバーイベント解除（管理者用） |
| `action <action_id> [duration_minutes]` | アクション実行。可変時間アクションでは `duration_minutes` を分単位で指定 |
| `use-item <item_id>` | 所持アイテムを使用 |
| `wait <duration>` | 待機（10分単位の整数 `1`〜`6`） |
| `conversation-start <target_agent_id> <message>` | 会話開始 |
| `conversation-accept <message>` | 会話受諾 |
| `conversation-reject` | 会話拒否 |
| `conversation-join <conversation_id>` | 進行中の会話に参加（反映は次のターン境界） |
| `conversation-stay` | inactive_check に残留応答 |
| `conversation-leave [message]` | inactive_check に離脱応答 |
| `conversation-speak <next_speaker_agent_id> <message>` | 会話発言 |
| `conversation-end <next_speaker_agent_id> <message>` | 会話終了 / 退出 |
| `map` | マップ全体取得 |
...
- `move`
- `action`（可変時間アクションでは `duration_minutes` を受け付ける）
- `use_item`
- `wait`（`duration` は10分単位の整数 `1`〜`6`）
- `conversation_start`
- `conversation_accept`
- `conversation_join`
- `conversation_stay`
