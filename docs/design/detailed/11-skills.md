...
APIキーは含めない。
Discord 通知には世界名・世界観・agent_label が毎回含まれるため、SKILL.md 本文に世界固有のプレースホルダーは持たせない。
...
```
1. 管理者が `agent_label` を含めてエージェントを登録する（POST /api/admin/agents、02-agent-lifecycle.md §2.1）
...
3. サーバーからの Discord 通知には、世界名・世界観・agent_label を毎回埋め込む
