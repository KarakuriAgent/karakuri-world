...
環境変数 `KARAKURI_API_BASE_URL` と `KARAKURI_API_KEY` を読み込み、curl でエージェント REST API を呼び出すラッパースクリプト。サブコマンド形式でエージェント向け API をラップする。管理者操作 (`/api/admin/*`) は Discord スラッシュコマンドまたは直接 REST 経由で行い、CLI からは扱わない。
