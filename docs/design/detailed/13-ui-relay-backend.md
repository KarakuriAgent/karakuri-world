...
```typescript
// 本体 src/types/conversation.ts の ConversationClosureReason をそのまま import して使う。
// - conversation_closing.reason は本体 event.ts で Extract<ConversationClosureReason, 'ended_by_agent' | 'max_turns' | 'server_event'> として 3 値に narrow されている
// - conversation_ended.reason は ConversationClosureReason 全 5 値（'turn_timeout' / 'participant_logged_out' を含む）
// mirror は両方の reason を一時保持しうるため、narrow されない元型をそのまま採用する。
// これにより本体側で ConversationClosureReason に値が追加されても型が自動追従し、DO 側の drift を防ぐ。
import type { ConversationClosureReason } from '<本体 src/types/conversation>';
...
interface BridgeState {
  ...
  publish_failure_streak: number;   // R2 publish の連続失敗回数。成功 publish で 0 にリセットする
  heartbeat_failure_streak: number; // heartbeat refresh の連続失敗回数。成功で 0 にリセットする
  disconnect_started_at?: number; // /ws 切断開始時刻。再接続成功で undefined に戻す
}
```
...
R2 書き込み失敗時は `latest_snapshot` / `last_refresh_at` を巻き戻さず、`publish_failure_streak` を +1 して `publish_alarm_at = max(Date.now(), (last_publish_at ?? 0) + backoff)` を再設定する。ここで `backoff = min(SNAPSHOT_PUBLISH_INTERVAL_MS * 2^(publish_failure_streak - 1), PUBLISH_BACKOFF_MAX_MS)`（`PUBLISH_BACKOFF_MAX_MS` 既定 60000）とし、広域障害時に 5 秒毎の永続リトライで CPU / class A 操作が暴走しない形にする。publish 成功時は `publish_failure_streak = 0` にリセットする。これにより、初回 publish が失敗した場合でも次の world event を待たずに retry が継続しつつ、連続失敗時の呼び出し頻度には上限が掛かる。

heartbeat refresh も同様に `heartbeat_failure_streak` を維持し、連続失敗時は `heartbeat_alarm_at = Date.now() + min(SNAPSHOT_HEARTBEAT_INTERVAL_MS * 2^(heartbeat_failure_streak - 1), HEARTBEAT_BACKOFF_MAX_MS)`（既定上限 5 分）で再予約する。成功で streak は 0 にリセットする。
...
- **切断中のイベントは履歴に残らない**（Phase 1 時点の既知の仕様上の欠落）。`/ws` 切断中に本体で発火した `event`（`movement_*` / `action_*` / `conversation_*` など）は relay に届かないため、`world_events` / `world_event_agents` / `world_event_conversations` には insert されず、`/api/history` にも現れない。heartbeat や再接続後初回 `snapshot` により世界の現状は復元されるが、切断期間内のタイムラインは観戦 UI 上で gap になる。運用者はこの gap の可能性を `relay.ws.event_gap_ms` / `relay.ws.connect_duration_ms` と `relay.ws.disconnect_total{handshake_status}` から推定する
- `server_event_fired` については、再接続後の snapshot refresh で `WorldSnapshot.server_events` と `recent_server_events` の mirror を突合するため、まだ outstanding な分は部分的に状態として復元される。ただし切断中に発火して短時間で outstanding から外れたイベントは同じく履歴から欠落する
- 切断期間は `BridgeState.disconnect_started_at` で記録し、再接続成功時に `relay.ws.connect_duration_ms` / `relay.ws.event_gap_ms` として出力する。切断 → 再接続の replay 機構（last_event_id / resume token 方式）は本設計の範囲外で、将来拡張として温存する
...
| 指標 | 種別 | 増分契機 |
|------|------|----------|
| `relay.ws.disconnect_total{reason,handshake_status}` | counter | `/ws` 切断ごと。`reason` は `close` / `error` / `idle`、`handshake_status` は upgrade HTTP ステータスを分類した `auth_rejected` / `not_found` / `server_error` / `network` / `timeout` / `server_close` |
| `relay.ws.connect_duration_ms` | gauge | `/ws` 再接続成功時の `Date.now() - disconnect_started_at`。構成不備による長期切断の可視化に使う |
| `relay.ws.event_gap_ms` | gauge | `/ws` 再接続成功時の切断期間。切断中に本体で発火したイベントが履歴に残っていない可能性がある範囲を運用者が推定するための参考値 |
...
| `relay.r2.publish_failure_streak` | gauge | R2 publish の連続失敗回数。暴走リトライブレーキが掛かっている区間を示す |
| `relay.heartbeat.failure_streak` | gauge | heartbeat refresh の連続失敗回数 |
| `relay.d1.ingest_failure_total{event_type}` | counter | event ingest batch の失敗ごと（event_type は対象イベント種別） |
| `relay.d1.retention_run_total{result}` | counter | retention cron 実行ごと（`result`: `success` / `failure`） |
| `relay.d1.retention_deleted_rows` | gauge | 直近 retention 成功時に削除した行数（`world_events` + `server_event_instances` 合算） |
...
少なくとも `relay.d1.ingest_failure_total` と `relay.r2.publish_failure_total` には運用アラート（X 分間に N 件超で通知）を設定し、履歴欠落および snapshot 配信停止が無言で蓄積するのを防ぐ。`relay.d1.retention_run_total{result=success}` が N 日観測されない場合は「retention cron が無音で停止している」として通知する。`relay.r2.publish_failure_streak` が上限待機（60 秒）に張り付く状態も通知対象に含める。`handshake_status = auth_rejected` が持続する切断は構成不備として即時ページングし、`network` / `timeout` 系は一定時間連続する場合のみ通知するなど経路別に分類する。observability ターゲットの未整備のまま本番配備しないこと。
