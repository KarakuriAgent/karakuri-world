...
2. `UIBridgeDurableObject` は受け取った body をそのまま `applySnapshot` に流し `SpectatorSnapshot` へ変換、`snapshot/manifest.json`・`snapshot/v/{generated_at}.json`（互換用に `snapshot/latest.json` も可）と history document を R2 へ publish する
...
5. DO 側に pull/resync 経路は持たない。コールドスタート時は永続化済みの `latest_snapshot` を復元し、次回の event-driven push を待つ

エージェントの行動サイクルが 10 分単位であることを踏まえ、観戦 UI の鮮度要件は「イベント発生後に manifest 経由で数十秒以内に追随すること」で十分である。current-state の primary path は manifest + versioned snapshot を CDN 経由で読む構成であり、クライアント向け WebSocket 配信は行わない。publish path は body push + R2 PUT retry で失敗耐性を確保する。
...
- **snapshot 更新戦略**: backend の `SnapshotPublisher` が可視ステート変化イベントごとに `WorldSnapshot` を body として push し、`UIBridgeDurableObject` がそれを `SpectatorSnapshot` へ変換して `manifest + versioned snapshot` を R2 に書き出す
- **障害時の挙動**: primary path は event-driven publish を前提とする。push 側は debounce + 指数バックオフ retry、DO 側は R2 PUT failure 時のみ alarm retry を行う。cold start 時は永続化済みの直前 snapshot を restore し、次回 push を待つ。history 側に gap が生じても current-state 配信は継続し、`/ws` には依存しない
