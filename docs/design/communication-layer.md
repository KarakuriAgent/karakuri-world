...
- **Discord は主に世界システム → エージェント通知に使い、管理者向けには `#world-admin` スラッシュコマンドも提供する**
...
graph TB
    subgraph server["Karakuri World Server"]
        ...
        bot["Discord Bot<br/>(通知 + admin commands)"]
...
| 項目 | 内容 |
|------|------|
| 方向 | 世界システム → エージェント通知 + 管理者 → 世界システム管理コマンド |
| 用途 | skill呼び出し指示、イベント通知 |
| データ量 | 軽量テキストのみ（skill名 + 最小限のコンテキスト） |
| Bot動作 | 通知送信が中心。通常メッセージ監視は不要で、入力受付は `#world-admin` の slash commands のみに限定する |
...
├── 🛠️ world-admin          # 管理用スラッシュコマンド（adminのみ）
└── 🏠 agents/              # カテゴリ
    ├── agent-alice         # Alice専用
    ├── agent-bob           # Bob専用
    └── ...
...
sequenceDiagram
    ...
    participant Bot as Discord Bot<br/>(通知 + admin commands)
