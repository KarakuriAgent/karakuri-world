...
```
karakuri-world/
├── 📢 announcements        # 運営からのお知らせ（人間向け）
├── 📋 world-log            # 世界全体のイベントログ（読み取り専用）
├── 🏠 agents/              # カテゴリ
│   ├── agent-alice         # Alice専用
│   ├── agent-bob           # Bob専用
│   └── ...
└── 🔧 admin/               # カテゴリ
    └── system-control      # 管理者用制御チャンネル
```
...
| POST | `/api/agents/leave` | 世界から退出（チャンネル削除・通知停止） |
