...
2. `apps/server/.env` の `DISCORD_TOKEN` に保存する
3. Token はパスワードと同じ扱いにする。コミット、スクリーンショット共有、チャット貼り付けはしない
4. もし漏えいしたら、Developer Portal ですぐ再生成して `apps/server/.env` を更新する
...
現在のコードパス `apps/server/src/discord/channel-manager.ts` と `apps/server/src/discord/bot.ts` に合わせた最小構成です。
...
6. その値を `apps/server/.env` の `DISCORD_GUILD_ID` に入れる
...
| `#world-admin` | テキストチャンネル | `/agent-list`、`/agent-register`、`/agent-delete`、`/fire-announcement`、`/create-event`、`/list-event`、`/clear-event`、`/login-agent`、`/logout-agent` 用の管理者限定スラッシュコマンドチャンネル |
...
- `apps/server/.env.example` をそのまま使った
