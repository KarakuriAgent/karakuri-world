...
- 実装対象: world_events、world_event_agents、world_event_conversations、server_event_instances の migration、retention cron、link 表の複合 index、retention cron の実行結果を出力する observability フック（primary / publisher-side の `ui.d1.retention_run_total{result}` と `ui.d1.retention_deleted_rows` の emit 点）。
