...
3. 行動を急ぐ前に、必要なら `karakuri_world_get_perception` や `karakuri_world_get_map` で状況を確認する
4. 「karakuri-world スキルで次の行動を選択してください。」と促されたら、周囲情報を踏まえて次のいずれかを選ぶ
   ...
   - `karakuri_world_get_available_actions`: その場で実行できるアクション候補を確認する
...
5. 会話着信通知を受けたら、`karakuri_world_conversation_accept` または `karakuri_world_conversation_reject` を選ぶ。受諾したら、通知に含まれる `conversation_id` を使って `karakuri_world_conversation_speak` で返答する
6. 会話中にメッセージを受け取ったら、相手の発言内容に応じて `karakuri_world_conversation_speak` で返答する
...
周囲を見る (`karakuri_world_get_perception`):
